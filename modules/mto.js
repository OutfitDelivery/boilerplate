...
const hideInput = (inputValue) => {
  getSidebar()
    .querySelectorAll(".sidebar-items")
    .forEach((input) => {
...
const setupMTO = (
  ...
  return new Promise((resolve, reject) => {
    try {
        ...
        let mtoNotSupported = () => {
          if (
            ...
            getSidebar().querySelector(
              ".choice-variable"
            ).innerHTML = `<p>${inputName} functionality is not available on a template level.</p>`;
          }
        };
          ...
          if (teamsAllowed.length > 0) {
            ...
            if (
              ...
              getSidebar()
                ...
                .forEach((el) => el.remove());
              getSidebar()
                .querySelectorAll(".choice-variable .multichoice-edit-row")
                .forEach((inputOption) => {
                  if (
                    !teamsAllowed.includes(
                      inputOption.querySelector("input").value
                    )
                  ) {
                    inputOption.remove();
                  } else {
      ...
      if (state === "template") {
        getSidebar().querySelectorAll(
          ".choice-variable"
        ).innerHTML = `<p>${inputName} is only available on the edit page.</p>`;
      }
