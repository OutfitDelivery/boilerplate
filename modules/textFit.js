...
function processItem(el, settings) {
  ...
  const textFittedSpan = el.querySelector('span.textFitted');
  ...
  const maxLine = parseInt(el.dataset.maxLine || settings.maxLine);
  // console.log(maxLine, 'maxline')
  const startingSize = innerSpan.style.fontSize;
  ...
  while (low <= high) {
    ...
    if (Number.isInteger(maxLine)) {
      const lineCount = countLines(innerSpan);
  ...
  if (Number.isInteger(maxLine) || settings.stopOverflow) {
    ...
    if (settings.stopOverflow) {
      const overflow = getHeight(el) < getHeight(innerSpan);
      if (overflow) {
        el.classList.add('overflow');
      }
    }
  ...
  if (settings.alignVert) {
    // addStyleSheet();
    const height = getHeight(innerSpan);
